...
# Su pirámide es un poco rara, ya que en realidad es una pared en forma de pirámide - es plana. La pirámide se apila de acuerdo con un 
# principio simple: cada capa inferior contiene un bloque más que la capa superior.
# La figura ilustra la regla utilizada por los constructores:
# Tu tarea es escribir un programa que lea la cantidad de bloques que tienen los constructores, y generar la altura de la pirámide 
# que se puede construir utilizando estos bloques.
...
bloscks_para_fila = 1
height = 0
while blocks >= bloscks_para_fila:
    blocks -= bloscks_para_fila
    height += 1
    bloscks_para_fila += 1
